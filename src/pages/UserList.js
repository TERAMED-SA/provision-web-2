...
const UserList = () => {
  ...
  const openSitesModal = async (userId) => {
    ...
    try {
      ...
      const sitesData = response.data.data?.data;
      ...
      const userSites = sitesData.filter(
        (site) => site.supervisorCode === employeeId
      );
  ...
  async function updateUser() {
    try {
      ...
      toast.info("Utilizador atualizado com sucesso");
      setTimeout(() => {
        window.location.reload();
      }, 3000);
    } catch (error) {
  ...
  const handleDeleteUser = async (userId) => {
    try {
      toast.warning("Não podes eliminar um utilizador de momento.");
    } catch (error) {
      console.error("Erro ao excluir usuário:", error.message);
    }
  };
  ...
  return (
    <div className="container4">
      ...
      <div className="container-fluid">
        <Link to="/Home" className="p-1">Início </Link> / <span>Utilizadores</span>
        <br></br> <br></br>
        <div className="space">
          <div className="">
            <input
          ...
          {!isLoading && filteredUsers.length > 0 && (
            <>
              <DataGrid
                    ...
                    renderCell: (params) => (
                      <div className="central mb-3">
                        ...
                        <button
                          ...
                          onClick={() => openSitesModal(params.row.idUser)}
                        >
                          <FontAwesomeIcon icon={faBuilding} /> Sites
                        </button>
                      </div>
                    ),
                  },
                ...
                pagination
                onPageSizeChange={handleUsersPerPageChange}
        ...
        {/* Modal for Viewing Sites */}
        <Modal
          isOpen={isSitesModalOpen}
          onRequestClose={() => setIsSitesModalOpen(false)}
          contentLabel="Sites Supervisionados"
          className="custom-modal"
          overlayClassName="custom-modal-overlay"
        >
          ...
          <div className="modal-actions">
            <button className="btn btn-secondary" onClick={closeSitesModal}>
