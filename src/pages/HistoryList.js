...
function HistoryList() {
  ...
  async function fetchHistory() {
    try {
      ...
      const filterSite = await responseClient.data.data.data.filter(
        (code) => code.clientCode === getCompany
      );
      const formattedHistoryList = await response.data.data.data.map(
        (history) => ({
          ...history,
      ...
      const filteredHistoryList = formattedHistoryList.filter((history) => {
        return filterSite.some(
          (site) => site.costCenter === history.costCenter
        );
      });
